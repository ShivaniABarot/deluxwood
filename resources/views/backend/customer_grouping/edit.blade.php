...
<div class="row align-items-center">
		<div class="border-0 mb-4">
			<div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
				<h3 class="fw-bold mb-0">Edit Customer Grouping</h3>
				 <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{url('admin/home')}}">Home</a></li>
                            <li class="breadcrumb-item"><a href="{{url('admin/customer-grouping/index')}}">Customer Grouping</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Edit Customer Grouping</li>
                        </ol>
                    </nav>
			</div>
		</div>
	...
	<div class="row clearfix g-3">
		...
		<div class="col-lg-12 d-flex justify-content-center align-items-center border-0 rounded-lg auth-h200">
			<div class="w-100 p-3 p-md-5 card border-0 shadow-sm" style="max-width: 100rem;">
				<!-- Form -->
				<form class="row g-1 p-3 p-md-4" action="{{url('admin\customer-grouping\update')}}\{{$customer->customer_id}}" enctype="multipart/form-data"  method="POST" id="CustomerGroupingForm">
                @method('PATCH')
                @csrf
                   ...
                   <div class="col-6">
                        <div class="mb-2">
                            <label class="form-label">Select Group Type <span class="text-danger"> *</span></label>
                            <select class="form-control category form-select" name="customer_group_id">
                                <option value="">Please Select</option>
                                 @foreach ($customer_group_type as $val)
                                  @if($val->customer_group_id  == $customer->customer_group_id)
                                  <option selected value="{{$val->customer_group_id}}">{{ $val->group_title}}</option>
                                  @else
                            ...
                            <span class="text-danger kt-form__help error customer_group_id"></span>
                        </div>
                    </div>
