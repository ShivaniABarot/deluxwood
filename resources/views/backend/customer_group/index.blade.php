	...
	<div class="row align-items-center">
		<div class="border-0 mb-4">
			<div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
				<h3 class="fw-bold mb-0">Customer Group List</h3>
			...
			<div class="card">
				<div class="card-body">
					<table id="myDataTable" class="table table-hover align-middle mb-0" style="width: 100%;">
						...
						<tbody>
							@foreach($customer_group as $val)
							<tr>
								<td><strong>{{$val->customer_group_id}}</strong></td>
								<td>{{$val->group_title}}</td>
								<td>{{$val->group_dicount_percent}}%</td>
								<td>{{$val->group_description}}</td>
								<td>
									<div class="btn-group" role="group" aria-label="Basic outlined example">
										<a href="{{url('admin/customer-group/edit')}}/{{$val->customer_group_id}}" class="btn btn-outline-secondary"><i class="icofont-edit text-success"></i></a>
										<a href="javascript:void(0)" onclick="deleteModal('{{$val->customer_group_id}}');" class="btn btn-outline-secondary"><i class="icofont-trash text-danger"></i></a>
