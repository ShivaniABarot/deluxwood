...
class DraftController extends Controller
{
   ...
   {
      $customerDraftStyle=CustomerDraftStyle::where('customer_draft_id',$request->draft_id)->where('door_style_Id',$request->doorStyle_id)->first();
    ...
    public function editModificationInfo(Request $request)
{
    ...
    if($size != ""){ $existingRecord->size = $size; }else{ $existingRecord->size = null;}
    if($message != ""){$existingRecord->message= $message; }else{ $existingRecord->message = null;}
    $existingRecord->save();
...
public function count_price($id , $quantity ,$message){
    ...
    $user = Auth::user()->id;
      ...
      $product = DraftProduct::leftJoin('product_item', 'draft_product.product_id', '=', 'product_item.product_id')
           ...
           ->where('draft_product.is_shipping_cost', "No")
           ->select('draft_product.*','product_master.*','customer_draft_style.*',
                    'product_item.description as product_description','customer_draft_style.configuration as door_style_configuration',
