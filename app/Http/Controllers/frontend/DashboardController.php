...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if ($user) {
            ...
            // Log drafts for debugging
            Log::info('Drafts retrieved for user ' . $user->id, ['drafts' => $customer_draft->toArray()]);

            return view('frontend.dashboard', compact('customer_draft', 'pagename'));
        }

        $customer_draft = collect(); // Empty collection for non-authenticated users
        return view('frontend.dashboard', compact('customer_draft', 'pagename'));
    }
    ...
    public function duplicate(Request $request, $id)
    {
        try {
            ...
            $newDraft->po_number = 'CO-' . $originalDraft->po_number;
            $newDraft->customer_id = $user->id;
            $newDraft->draft_status = $originalDraft->draft_status;
            $newDraft->created_at = now();
            $newDraft->updated_at = now();
            $newDraft->save();

            // Log the new draft creation
            Log::info('New draft created', [
                'new_draft_id' => $newDraft->customer_draft_id,
                'po_number' => $newDraft->po_number,
            ]);

            // Copy all associated styles using Eloquent Model
            $originalStyles = CustomerDraftStyle::where('customer_draft_Id', $id)->get();

            Log::info('Original styles for draft ID ' . $id, [
                'styles_count' => $originalStyles->count(),
                'styles' => $originalStyles->toArray(),
            ]);

            $lastInsertedStyleId = null;

            foreach ($originalStyles as $style) {
                // Check if door style exists
                $doorStyle = DoorStyle::find($style->door_style_Id);

                if ($doorStyle) {
                    try {
                        // Use Eloquent model instead of raw query
                        $newCustomerDraftStyle = new CustomerDraftStyle();
                        $newCustomerDraftStyle->customer_draft_Id = $newDraft->customer_draft_id;
                        $newCustomerDraftStyle->door_style_Id = $style->door_style_Id;
                        $newCustomerDraftStyle->configuration = $style->configuration;
                        $newCustomerDraftStyle->save();

                        $lastInsertedStyleId = $style->door_style_Id;

                        Log::info('Successfully inserted style using Eloquent', [
                            'new_draft_id' => $newDraft->customer_draft_id,
                            'door_style_Id' => $style->door_style_Id,
                        ]);
                    } catch (\Exception $e) {
                        Log::error('Failed to insert style using Eloquent', [
                            'new_draft_id' => $newDraft->customer_draft_id,
                            'door_style_Id' => $style->door_style_Id,
                            'error' => $e->getMessage(),
                        ]);
                    ...
                    Log::warning('No matching door style found for door_style_Id ' . $style->door_style_Id);
                }
            }

            // Fetch style details for the response
            $doorStyle = DoorStyle::find($lastInsertedStyleId);
            ...
            Log::error('Error duplicating draft ID ' . $id, [
    ...
    {
        $product_id = ProductDoorStyle::where('door_style_id', $request->doorStyle_id)
            ->pluck('product_id')
            ->toArray();
    
        $draft_products = DraftProduct::where('customer_draft_Id', $id)
        ...
        $existingDraftStyle = CustomerDraftStyle::where('door_style_Id', $request->doorStyle_id)
