...
class PDFController extends BaseController
{
    public function sticker($id)
    {
        ...
        $customerDraftStyles = CustomerDraftStyle::where('customer_draft_id', $id)
            ->get();
    ...
    //  return view('backend.pdf.sticker',$data);

        // return $pdf->download($pdf_data->client_name . '_' . $pdf_data->po_number . '.pdf');
        $pdf = PDF::loadView('backend.pdf.sticker', $data);
        $pdf->setPaper('L');
        $pdf->output();
        $domPdf = $pdf->getDomPDF();
        $canvas = $domPdf->get_canvas();
        ...
        // if($customer_draft->payment_method == "Pay Later")
        // {
        //     $canvas->page_text($canvas->get_width() / 3, $canvas->get_height() / 3, 'Pay Later', null, 80,  [0.398, 0.000, 0.455, 0.043], 2, 2, -30 ,['opacity' => 0.1]);
        // }else{
        // $canvas->page_text($canvas->get_width() / 3, $canvas->get_height() / 3, 'PAID', null, 80,  [0.398, 0.000, 0.455, 0.043], 2, 2, -30 ,['opacity' => 0.1]);
        // }
        return $pdf->download($pdf_data->client_name . '_' . $pdf_data->po_number . '.pdf');

    }
    public function withPrice($id)
    {
    ...
    public function withoutPrice($id)
    {
